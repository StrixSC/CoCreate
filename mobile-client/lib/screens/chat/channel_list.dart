...
class ChannelListView extends StatefulWidget {
  ...
  @override
  _ChannelListViewState createState() =>
      _ChannelListViewState(username: this.username);
}
...
class _ChannelListViewState extends State<ChannelListView> {
  ...
  Widget channelListWidget() {
    return (Column(children: [
      ...
      MediaQuery.removePadding(
          context: context, removeTop: true, child: SizedBox()),
      const Divider(thickness: 2, color: Colors.black),
      Row(
  ...
  createChannelDialog() async {
    ...
    setState(() {
      ...
      //   isActive: false,
      // ));
    }); // Add check for channel already existing
  }
  ...
  Widget channelChatWidget() {
    ...
    IO.Socket socket = IO.io(
        ...
        IO.OptionBuilder()
            .setExtraHeaders({
              'Cookie':
                  'connect.sid=s%3AqWwIIJbxKIx6WtufUOJknXHlsE6UxUwn.ICblW%2FBsjBpzsSsRW7YMN6PFUkMxBohe%2BoTT3hQwwns; Path=/; Domain=localhost; HttpOnly; Expires=Mon, 25 Oct 2021 18:40:54 GMT;'
            })
            .disableAutoConnect()
    ...
    socket.on('connect', (_) {
      ...
      socket.emit('join-channel',
          {'channelId': "30a27c9c-4426-48b0-a08d-2a8c1aa143cb", 'userId': ""});
    });

    return channelChatWidget();
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Expanded(
            child:
                isChannelSelected ? channelChatWidget() : channelListWidget()),
      ],
    );
  }
}
