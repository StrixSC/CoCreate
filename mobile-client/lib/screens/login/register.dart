...
class _RegisterState extends State<Register> {
    ...
    if (response.statusCode == 201) {
      ...
      return showDialog<String>(
          ...
          builder: (BuildContext context) => AlertDialog(
            ...
            actions: <Widget>[
              TextButton(
                onPressed: () { Navigator.pop(context, 'Ok'); Navigator.pushNamed(context, LoginRoute);},
                child: const Text('Ok'),
              ),
            ],
          ));
  ...
  textForm(hintText, controller) {
    return hintText != "Mot de Passe" ? TextFormField(
      ...
      onFieldSubmitted: (value) {
        if (_formKey.currentState!.validate()) {
          Map userInfo = {'email': emailController.text, 'password': passController.text, 'username' : usernameController.text,
