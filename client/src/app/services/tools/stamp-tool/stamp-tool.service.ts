...
export class StampToolService implements Tools {
  ...
  constructor(
    ...
    this.facteurSize = new FormControl(
      INITIAL_SCALE,
      Validators.min(MIN_FACTOR_VALUE)
    );
  ...
  onPressed(event: MouseEvent): void {
    if (event.button === LEFT_CLICK && this.stampSvgString.value) {
      ...
      this.stamp = {
        x,
        y,
        sizeFactor: this.facteurSize.value,
        svgString: this.stampSvgString.value,
        angle: this.angleRotation,
      };

      this.stampCommand = new StampCommand(
  ...
  private setAngleForward(): void {
    this.angleRotation =
      (this.intervaleDegresRotation + this.angleRotation) % 360;
    if (this.stampCommand) {
      this.stampCommand.setAngle(this.angleRotation);
    }
  }
