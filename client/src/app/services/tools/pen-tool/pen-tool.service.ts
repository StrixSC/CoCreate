...
export class PenToolService implements Tools {
  ...
  constructor(
    ...
    this.minStrokeWidth = new FormControl(
      INITIAL_MIN_STROKE_WIDTH,
      Validators.min(VALIDATOR_MIN_STROKE_WIDTH)
    );
    this.maxStrokeWidth = new FormControl(
      INITIAL_MAX_STROKE_WIDTH,
      Validators.min(VALIDATOR_MIN_STROKE_WIDTH)
    );
  ...
  onPressed(event: MouseEvent): void {
    if (event.button === RIGHT_CLICK || event.button === LEFT_CLICK) {
      if (
        this.maxStrokeWidth.valid &&
        this.minStrokeWidth.valid &&
        this.maxStrokeWidth.value >= this.minStrokeWidth.value
      ) {
        ...
        this.penCommand = new PenCommand(
  ...
  private getWidth(event: MouseEvent): void {
    ...
    this.width = Math.max(
      this.maxStrokeWidth.value *
        (1 - Math.max(Math.log(this.travelDistance), 0) / WIDTH_SCALE_FACTOR),
