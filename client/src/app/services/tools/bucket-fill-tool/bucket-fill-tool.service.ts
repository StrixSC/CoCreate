...
export class BucketFillToolService implements Tools {
  ...
  constructor(
    ...
    this.tolerance = new FormControl(0, [
      Validators.min(0),
      Validators.max(100),
    ]);
    ...
    return new Promise<void>((res, rej) => {
      if (event.button === RIGHT_CLICK || event.button === LEFT_CLICK) {
        const snackBarRef = this.snackBar.open(
          "L'outil remplissage est en train de remplire votre région",
          undefined,
        ...
        img.onload = () => {
          ...
          const floodResult: { imgData: ImageData; min: Point; max: Point } =
            this.flood(
              imageData,
              offset,
              this.getPixelColor(imageData, offset),
              {
                rgb:
                  event.button === LEFT_CLICK
                    ? this.colorTool.primaryColor
                    : this.colorTool.secondaryColor,
                a:
                  event.button === LEFT_CLICK
                    ? this.colorTool.primaryAlpha
                    : this.colorTool.secondaryAlpha,
              }
            );
  ...
  private flood(
    ...
    targetColor: RGB,
    fillColor: RGBA
  ): { imgData: ImageData; min: Point; max: Point } {
    ...
    while (toVisit.length > 0) {
      ...
      for (let offset = -1; offset <= 1; offset += 2) {
        ...
        if (
          this.isInDrawing(xOffset, currentPoint.y, width, height) &&
          !visited.has(xOffset + "_" + currentPoint.y) &&
          this.colorMatch(
            this.getPixelColor(imageData, {
          ...
          this.changePixelColor(
            imageDataToReturn,
            { x: xOffset, y: currentPoint.y },
            fillColor
          );
          visited.add(xOffset + "_" + currentPoint.y);
        }
        if (
          this.isInDrawing(currentPoint.x, yOffset, width, height) &&
          !visited.has(currentPoint.x + "_" + yOffset) &&
          this.colorMatch(
            this.getPixelColor(imageData, { x: currentPoint.x, y: yOffset }),
            targetColor
          )
        ) {
  ...
  private changePixelColor(
    ...
    imageData.data[offsetImage + A_OFFSET] = Math.ceil(
      changeColor.a * RGB_MAX_VALUE
    );
  }
